...
class Xcrud
{
    ...
    public static function get_requested_instance()
    {
            ...
            $key)
        {
            ...
            self::error('<strong>A chave de verificação está desatualizado</strong><br />
                Isto significa que o cache do seu navegador está com uma  versão anterior desta página (por razões de segurança a chave de verificação é gerado a cada pedido)<br />
                Por quê? Talvez você pressionou o botão voltar no seu browser ou foi aberto um marcador da última sessão. <br /> <strong> Apenas recarregue a página, não aconteceu nada :)</strong>');
    }
    ...
    public function unset_category($bool = true)
    {
        $this->is_create = !(bool)$bool;
        return $this;
    }
    ...
    protected function render_button_category($name = '', $pagina = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        $out = '';
        if (isset($this->{'is_' . $after}) && !$this->{'is_' . $after})
        {
    ...
    protected function add_button_category( $pagina, $class = '', $icon = '')
    {
      if ($this->is_create && !isset($this->hide_button['add']) && !$this->table_ro)
      {
      return $this->render_button_category('Categoria' , $pagina , $task='' , $after='', $class='remove_action btn btn-primary' , $icon="glyphicon glyphicon-plus-sign" , $mode='' );
    }
    }
