...
function publish_action_noticia($xcrud)
{
    if ($xcrud->get('primary'))
    {
        $db = Xcrud_db::get_instance();
				$query = 'UPDATE noticias SET `ativo` = b\'1\' WHERE id_noticia = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}
function unpublish_action_noticia($xcrud)
{
    if ($xcrud->get('primary'))
    {
        $db = Xcrud_db::get_instance();
        $query = 'UPDATE noticias SET `ativo` = b\'0\' WHERE id_noticia = ' . (int)$xcrud->get('primary');
        $db->query($query);
    }
}


function save_action($xcrud)
{
